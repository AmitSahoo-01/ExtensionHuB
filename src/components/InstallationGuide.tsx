...
export default function InstallationGuide() {
  const steps: Step[] = [
    {
      ...
      icon: <DownloadCloud className="w-6 h-6 text-purple-400" />
    },
    {
      number: "02",
      ...
      icon: <FolderOpen className="w-6 h-6 text-pink-400" />
    },
    {
      number: "03",
  ...
  return (
    <section id="installation" className="relative py-24 md:py-32 bg-[#09090b]">
      ...
      <div className="absolute top-1/2 left-1/4 w-[400px] h-[400px] rounded-full bg-purple-900/10 blur-[150px] pointer-events-none z-0" />
      <div className="absolute bottom-0 right-10 w-[300px] h-[300px] rounded-full bg-indigo-900/10 blur-[120px] pointer-events-none z-0" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div className="hidden lg:block absolute top-[90px] left-[5%] right-[5%] h-[2px] bg-gradient-to-r from-purple-500/20 via-pink-500/20 to-indigo-500/20 -z-10" />

          {steps.map((step, idx) => (
            <motion.div
              key={idx}
              variants={itemVariants}
              className="glass-card rounded-3xl p-8 relative flex flex-col justify-between group overflow-hidden border border-purple-500/10 hover:border-purple-500/30 transition-all duration-300 shadow-md"
            >
              {/* Giant background step number */}
              <div className="absolute right-6 top-4 font-mono font-black text-6xl text-purple-500/5 group-hover:text-purple-500/10 transition-colors pointer-events-none select-none">
                {step.number}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="w-12 h-12 rounded-2xl bg-zinc-900 border border-purple-500/15 flex items-center justify-center shadow-inner group-hover:border-purple-500/40 transition-all duration-300">
                    {step.icon}
                  </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-purple-500 to-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-16 glass-card rounded-2xl p-6 border border-purple-500/20 max-w-3xl mx-auto flex items-start gap-4 shadow-xl"
        >
          <div className="text-2xl mt-0.5">💡</div>
          <div>
            <h4 className="text-sm font-bold text-purple-300 mb-1">Developer Pro-Tip</h4>
            <p className="text-xs text-zinc-400 leading-relaxed">
              When updating an unpacked extension, you do not need to repeat these steps! Simply click the <strong className="text-white">Refresh Icon</strong> (🔁) on the extension card inside <code className="bg-zinc-900 px-1 py-0.5 rounded border border-zinc-800 text-purple-400 text-[10px]">chrome://extensions</code> to reload the latest code changes instantly.
            </p>
          </div>
        </motion.div>
